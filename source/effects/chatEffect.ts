// @ts-nocheck
import {
  createBlockingEffect as createEffect,
  Observable,
  after,
  THRESHOLD,
...
function getLLMStream(userMessage: UserMessage): Observable<Chunk> {
  // 5. TODO Notify of any error from making the API call
  return new Observable((notify) => {
    ...
    openai.chat.completions
      .create({
        model: "gpt-4.1",
        messages: [
          // 2. TODO include previous messages when sending the current one
          ...chatFx.state.value,
          {
            role: "user",
...
export const chatFx = createEffect<UserMessage, Chunk, Error, Message[]>(
  ...
  (req) => {
    return getLLMStream(req).pipe(
      concatMap((chunk) =>
        after(randomizePreservingAverage(THRESHOLD.AnimationShort), chunk)
      )
    );
  }
);
